...
void sendAllUser(void);//给所有在线的用户发送信息
pthread_t ntid;
	...
	while (1)
	{
		...
		{ 
			...
			if (chatclient.chat_command == SEND)//如果服务器成功接受到来自客户端的数据
			{
				...
				if (strcmp(logclient.username,chatclient.chat_receiveName) == 0||strcmp("admin",chatclient.chat_receiveName) == 0)//再次确认是不是给自己发送的信息
				{
					printf("%s\n",msgrecenote);//打印一下发送者发回的信息
					memset(recordfile,0,100);//写入问文件
					sprintf(recordfile,"%s-->%s\n",msgrecenote,userone.username);
...
void sendmessage(void)
{
	...
	number = getchar() - 49;
	getchar();
	printf("please input some content\n");
	fgets(msg,100,stdin);
    while (msg[i] != '\n')i++;
    msg[i] = '\0';
	...
	sprintf(recordfile,"%s：%s-->%s\n",userone.username,msg,chatclient.chat_receiveName);	
	fp = fopen("chatrecord", "a+");
	if (fp == NULL) 
	{
		perror("Open file recfile");
...
void sendAllUser(void)
{
    ...
    writeDataBuffer(&chatclient,sendbuf,MAXLINE);//写入缓存
   // printChat(chatclient);	
    msgse_flag = 1;//表示可以发送处理的数据
    sleep(1);
    msgse_flag = 0;//表示 不可以发送处理的数据
